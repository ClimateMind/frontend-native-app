...
function PersonalValueCard({ nr, value }: Props) {
  ...
  return (
    <Card style={{ padding: 10 }}>
      ...
      <CmTypography variant='h4' style={{ textAlign: 'left', fontSize: 18, paddingTop: 4 }}>{capitalizeFirstLetter(value.name)}</CmTypography>

      <View style={{ justifyContent: 'center', alignItems: 'center' }}>
        <PersonalValueIcon valueName={value.name} style={styles.image} />
      </View>
      
      <CmTypography variant='body' style={{ marginVertical: 10 }}>{value.shortDescription}</CmTypography>
      ...
      <Pressable onPress={() => setExpanded(prev => !prev)} style={{ marginTop: 20 }}>
        {!expanded && <CmTypography variant='button' style={styles.moreLessButton}>MORE</CmTypography>}
        {expanded && <CmTypography variant='button' style={styles.moreLessButton}>LESS</CmTypography>}
      </Pressable>
